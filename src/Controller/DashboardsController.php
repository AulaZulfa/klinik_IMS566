...
class DashboardsController extends AppController
{
	...
	public function index()
	{
		...
		//count doctors 
		$doctors = $this->fetchTable('Doctors');
		$total_doctors = $doctors->find()->count();

		//count appointments 
		$appointments = $this->fetchTable('Appointments');
		$total_appointments = $appointments->find()->count();
...
$doctor_percent = [];
foreach ($doctor_count as $type => $count) {
    $doctor_percent[$type] = round(($count / $total_doctors) * 100);
}
		//count patients 
		$total_patients = 27;
		...
		$this->set(compact(
			...
			'pending_faq_percent', 
			'pending_contact_percent', 'userLogs', 'formattedResults', 'totalActivityByMonth', 'todo_list'));
	}
}
